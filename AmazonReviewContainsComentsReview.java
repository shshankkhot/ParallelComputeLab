 ...
 */
public class AmazonReviewContainsComentsReview extends Configured implements Tool {
	// Just used for logging
	protected static final Logger LOG = LoggerFactory.getLogger(AmazonReviewContainsComentsReview.class);

	// This is the execution entry point for Java programs
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(HBaseConfiguration.create(), new AmazonReviewContainsComentsReview(), args);
		System.exit(res);
	}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(getConf(), "AmazonReviewContainsComentsReview");
		job.setJarByClass(AmazonReviewContainsComentsReview.class);
	...
	public static class MapReduceMapper extends TableMapper<Text, IntWritable> {
		...
		protected void setup(Context context) {
			...
			rowsProcessed = context.getCounter("AmazonReviewContainsComentsReview", "Rows Processed");
    		}
		...
		public void map(ImmutableBytesWritable rowKey, Result value, Context context) throws InterruptedException, IOException {
			...
			//	int image_cnt = jsonObject.get("image").getAsJsonArray().size();
				context.write(new Text("Rating:"+jsonObject.get("overall").getAsString()),one);
				...
				{
					context.write(new Text("Rating:"+jsonObject.get("overall").getAsString()+"-others"),one);
				}
